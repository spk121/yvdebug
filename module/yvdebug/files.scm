...
(define-module (yvdebug files)
  #:use-module (oop goops)
  #:use-module (mlg assert)
  #:use-module (yvdebug typelib)
  #:export (make-fileviewer
            attach-load-hook
            clear-fileviewer
            fileviewer?))

;; The FileList is an ordered list of (string, data) pairs.
;; It is kept in alphabetical order.

;; The FileViewer is a simple list viewer and a text view.  The list
;; view shows the filenames in the file list, and clicking on a file
;; name in the file list will create a text viewer of that file's
;; contents.

(define-class <FileList> ()
  ;; The actual list
  (lst #:init-value '() #:getter get-list #:setter set-list!)
  (lst-mutex #:init-thunk make-mutex #:getter get-list-mutex)

  ;; The updated flag is set to true when ever the list is changed and
  ;; is unset by the GUI when those changes have been visualized.
  (updated-flag #:init-value #f #:getter is-updated? #:setter set-updated!))

(define-method (update (FL <FileList>) (filename <string>))
  (let* ((original-list (get-list FL))
         (updated-list (assoc-set! original-list filename #t))
         (sorted-list (sort updated-list (lambda (a b) (string< (car a) (car b))))))
    (set-list! FL sorted-list)
    (set-updated! FL #t)))

(define-method (attach-load-hook (FL <FileList>))
  (set! %load-hook
    (lambda (filename)
      (update FL filename))))
